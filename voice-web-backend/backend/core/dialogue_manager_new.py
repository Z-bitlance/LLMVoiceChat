...
class DialogueManager:
    def __init__(self):
        ...
        try:
            ...
            print("成功加载AgentChat模块")
        except Exception as e:
            print(f"加载AgentChat失败: {e}，将使用模拟实现")
            # self.agent_chat = self._mock_agent_chat()
        ...
        try:
            ...
            print("成功加载MP3Player模块")
        except Exception as e:
            print(f"加载MP3Player失败: {e}，将使用模拟播放器")
            # self._mock_player()
    ...
    def process_user_input(self, text, speak=True):
        ...
        # 根据当前模式决定是否播放语音
        if self.voice_mode and speak:
            # 处理并生成语音但不在后端播放（前端将播放）
            audio_urls = self.speak(ai_response)
    ...
    def get_supported_roles(self):
        """获取支持的角色列表"""
        return [
            {"id": "libai", "name": "李白模拟", "voice": "longshu-v2"},
            {"id": "nekogirl", "name": "猫娘", "voice": "xiaoyun"},
